...
import {
  LayoutDashboard, FilePlus, Search, User as UserIcon,
...
const PATIENT_LINKS = [
  ...
  { name: 'Submit Request',  path: '/patient/submit-request', icon: FilePlus },
  { name: 'Track Request',   path: '/patient/track-request',  icon: Search },
...
export const PatientLayout = () => {
  ...
  if (user.role !== 'patient') return <Navigate to={`/${user.role}/dashboard`} replace />;

  const patientAvatar = user?.avatar_url || null;
    ...
    <div className="min-h-screen bg-[#F8F9FA] flex">
      ...
      >
        <div className="p-6 flex items-center gap-3 border-b border-white/5">
          ...
          <div>
            ...
            <p className="text-[10px] font-bold text-[#E74C3C] uppercase tracking-widest leading-3">Patient Portal</p>
          </div>
        </div>
        ...
        <div className="p-4 border-t border-white/5 space-y-1">
          <div className="flex items-center gap-3 px-4 py-3 rounded-xl bg-white/5">
            <div className="w-9 h-9 bg-slate-700 rounded-lg flex items-center justify-center overflow-hidden border border-white/10">
              {patientAvatar ? (
                <img src={patientAvatar} alt="Profile" className="w-full h-full object-cover" />
              ) : (
            ...
            <div className="min-w-0">
              ...
              <p className="text-[10px] text-[#E74C3C] font-bold uppercase tracking-widest">Medical</p>
            </div>
          </div>
          <button
      ...
      <div className="flex-1 md:ml-72 flex flex-col min-h-screen">
        <header className="h-16 bg-white/90 backdrop-blur-md border-b border-gray-100 shadow-sm fixed top-0 right-0 left-0 md:left-72 z-30 flex items-center justify-between px-4 md:px-8">
          ...
          <h2 className="hidden md:block text-lg font-black text-[#2C3E50]">Patient Portal</h2>
          <div className="flex items-center gap-3">
            <NotificationBell />
            <div className="flex items-center gap-2 pl-3 border-l border-gray-100">
              <div className="w-9 h-9 bg-slate-50 rounded-xl flex items-center justify-center overflow-hidden border-2 border-white shadow-md">
                {patientAvatar ? (
                  <img src={patientAvatar} alt="Avatar" className="w-full h-full object-cover" />
                ) : (
                  <UserIcon className="w-4 h-4 text-slate-400" />
                )}
              </div>
              <div className="hidden sm:block">
                <p className="text-xs font-black text-[#2C3E50] leading-3">{user?.name || 'User'}</p>
                <p className="text-[10px] text-[#E74C3C] font-black uppercase tracking-widest mt-1">Patient</p>
              </div>
            </div>
          </div>
        </header>
